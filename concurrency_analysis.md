...
```mermaid
graph TD
    A[OS Process] --> B["[Main Thread] Flask + Eventlet Server"]
    A --> C["[Monitor Thread] OS Level Threading.Thread"]
    
    subgraph "Server Engine (Eventlet Mode)"
        B -- "Websocket Only" --> B1["Client Dashboard"]
    end
    
    subgraph "Monitor Engine (Standard Networking)"
        C --> C1["asyncio.run (Isolated Loop)"]
        C1 --> C2["Playwright Driver (Fresh Instance)"]
        C2 --> C3["Proxy & DB Logic (Standard OS Sockets)"]
    end
...
### A. Hybrid Server Mode (No Monkey-Patching) [Synchronous]
**Mechanism**: Native networking with Eventlet Server Engine.
The monitor uses standard OS sockets for proxy connections and DNS, ensuring stability for proxy tunnels and avoiding "Dead Browser" crashes common in containerized environments.
*   **Type**: **Standard OS Networking**.
*   **Impact**: 100% stability for proxy tunnels and database connections.
*   **Why?**: Avoids networking interference caused by global green-thread patching.
...
Inside the background thread, we start a *private* `asyncio` event loop via `asyncio.run()`.
...
### G. WebSocket-Only Transport [Stability Fix]
**Mechanism**: Persistent stateful connection.
The system utilizes `transports=['websocket']` to eliminate redundant HTTP polling requests, significantly reducing the thread footprint on limited-resource environments.
*   **Type**: **Thread-Saving Optimization**.

### H. Resource Isolation (Fresh Browsers) [Memory Fix]
**Mechanism**: Launch-Check-Close pattern.
Each monitor check executes in a dedicated, temporary browser instance that is destroyed immediately upon completion to prevent memory accumulation.
*   **Type**: **Anti-Leak Pattern**.
...
| `detect_queue` | **Sync** | Analyzes page content string using scoring logic. |
| `detect_block` | **Sync** | Checks content for Imperva challenge signatures. |
| `run_monitor` | **Thread Entry** | Standard function that kicks off the Asyncio loop in a thread. |

---

## 4. Why this is "Railway-Ready"
By using `eventlet`, the program is highly memory-efficient. If you hit 100% CPU on Railway, the web dashboard will still remain responsive because Eventlet will "slice" time between the scraping work and the web traffic. The addition of `--disable-dev-shm-usage` and the **Bandwidth Saver** logic (blocking images/media via `page.route`) ensures that the **Asyncio** side uses minimal RAM and doesn't crash the container.
